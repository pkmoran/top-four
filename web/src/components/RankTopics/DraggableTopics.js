...
class DraggableTopics extends Component {
  topicIsRanked(topic) {
    return this.props.state === 'ranked' &&
      (topic.correctTopic || {}).status === 'ranked';
  }

  getItemStyle(isDragging, draggableStyle, topic) {
  ...
  getTopicTitle(topic) {
    ...
    if (ranked && !topic.isCorrect) {
      return <span>{topic.correctTopic.topic} <em><strike>{topic.topic}</strike></em></span>;
    }
  ...
  renderRevealButton(topic) {
    ...
    if (active && lockedIn && topic.status !== 'ranked') {
      return <Button variant="outlined" onClick={() => reveal(topic)}>Reveal</Button>;
    }
  }
                  ...
                  <Draggable isDragDisabled={this.props.isDragDisabled} key={topic.uid} draggableId={topic.uid} index={index}>
                    {(provided, snapshot) => (
                      ...
                      <div
                        ...
                        style={this.getItemStyle(snapshot.isDragging, provided.draggableProps.style, topic)}
                      >
